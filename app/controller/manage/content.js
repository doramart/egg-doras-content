...
const ContentController = {
  ...
  async create(ctx) {
    try {
      ...
      if (adminUserInfo && adminUserInfo.editor_id) {
        formObj.author_id = Number(adminUserInfo.editor_id);
      } else {
        if (ctx.service.user) {
          const existAdminUser = await ctx.service.user.item({
            query: {
              userName: adminUserInfo.userName,
            },
          });
          if (!_.isEmpty(existAdminUser)) {
            await ctx.service.adminUser.update(adminUserInfo.id, {
              editor_id: existAdminUser.id,
            });
            adminUserInfo.editor_id = formObj.author_id = existAdminUser.id;
          } else {
            // 创建一个虚拟马甲
            const thisAdminUser = await ctx.service.adminUser.item({
              query: {
                id: adminUserInfo.id,
              },
            });
            const { name, userName, logo, countryCode, phoneNum, email } =
              thisAdminUser;
            const virtualUser = await ctx.service.user.create({
              name,
              userName,
              logo,
              phoneNum,
            ...
            await ctx.service.adminUser.update(adminUserInfo.id, {
              editor_id: virtualUser.id,
            });
            adminUserInfo.editor_id = formObj.author_id = virtualUser.id;
          }
        } else {
